...
if [ $stage -le -2 ]; then
  ...
  awk -v power=$presoftmax_prior_scale_power -v smooth=0.01 \
     '{ for(i=1; i<=NF; i++) { count[i-1] = $i;  total += $i;}
        num_pdfs=NF;  average_count = total/num_pdfs;
     ...
     $egs_dir/pdf2num > $dir/presoftmax_prior_scale.vec
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $stage -le $x ]; then
    ...
    if [ $[$x%$diagnostic_period] == 0 ]; then
      ...
      $cpu_cmd JOB=1:$num_diagnostic_archives $dir/log/compute_prob_valid.$x.JOB.log \
