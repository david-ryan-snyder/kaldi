...
if [ $# != 1 ]; then
  ...
  echo " e.g.: $0 data/xvector_xent_1a"
  echo "This script trains the xvector system. You have to create the nnet"
  echo "configs and the egs first."
  echo ""
...
if [ ! -f $dir/nnet.config ]; then
  echo "$0: expected $dir/nnet.config to exist (e.g. run steps/nnet3/xvector/make_configs.py first)"
  exit 1
fi
...
if [ $stage -le -1 ]; then
  $cmd $dir/log/nnet_init.log \
    nnet3-init --srand=-1 $dir/nnet.config $dir/0.raw || exit 1
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $stage -le $x ]; then
    ...
    if [ $[$x%$diagnostic_period] == 0 ]; then
      ...
      $cpu_cmd JOB=1:$num_diagnostic_archives $dir/log/compute_prob_train.$x.JOB.log \
        nnet3-compute-prob --compute-accuracy=${compute_accuracy} $dir/$x.raw \
        "ark:nnet3-merge-egs --minibatch-size=$minibatch_size --measure-output-frames=false ark:$egs_dir/train_diagnostic_egs.JOB.ark ark:- |" &
    fi

    if [ $x -gt 0 ]; then
      $cpu_cmd $dir/log/progress.$x.log \
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        $cmd $train_queue_opt $dir/log/train.$x.$n.log \
          nnet3-train $parallel_train_opts --print-interval=10 \
