...
f = open(args.utt2int, "r");
if f is None:
    sys.exit("Error opening utt2int file " + str(args.utt2int));
spk2utt = {}
utt2spk = {}
num_utts = 0
for line in f:
...
def RandomUttAtLeastThisLong(spkr, min_length):
...
for archive_index in range(args.num_archives):
    ...
    for n in range(this_num_egs):
        print("len(spkrs) = " + str(len(spkrs)))
        if len(spkrs) == 0:
            print("Run out of speakers")
            break
        spkr = spkrs.pop()
        utt = RandomUttAtLeastThisLong(spkr, length)
        utt_len = utt2len[utt]
...
print("allocate_examples_xent.py: finished generating " + prefix + "ranges.* and " + prefix + "outputs.* files")
