...
for f in $fgmm_model $data/feats.scp ${data_bnf}/feats.scp; do
...
## Set up features.
feats="ark,s,cs:add-deltas $delta_opts scp:$sdata/JOB/feats.scp ark:- | apply-cmvn-sliding --norm-vars=${norm_vars} --center=true --cmn-window=300 ark:- ark:- | select-voiced-frames ark:- scp,s,cs:$sdata/JOB/vad.scp ark:- |"

feats_bnf="ark,s,cs:apply-cmvn-sliding --norm-vars=${norm_vars} --center=true --cmn-window=300 scp:${sdata_bnf}/JOB/feats.scp ark:- | select-voiced-frames ark:- scp,s,cs:$sdata/JOB/vad.scp ark:- |"
...
if [ $stage -le -1 ]; then
  ...
  $cmd2 $dir/log/gselect.$g.log \
    append-feats \
      "`echo $feats | sed s/JOB/$g/g`" \
      "`echo $feats_bnf | sed s/JOB/$g/g`" \
      ark:- \| \
    gmm-gselect --n=$num_gselect $dir/final.dubm ark,s,cs:- \
      "ark:|gzip -c >$dir/gselect.$g.gz" || exit 1 &
  done
  wait;

  for g in $(seq $nj_full); do
  $cmd2 $dir/log/post.$g.log \
    append-feats \
      "`echo $feats | sed s/JOB/$g/g`" \
      "`echo $feats_bnf | sed s/JOB/$g/g`" \
      ark:- \| \
    fgmm-global-gselect-to-post --min-post=$min_post $dir/final.ubm ark,s,cs:- \
    "ark,s,cs:gunzip -c $dir/gselect.$g.gz|"  ark:- \| \
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    for j in $(seq $nj_full); do
      Args[$j]=`echo "append-feats '$feats' '$feats_bnf' ark:- | ivector-extractor-acc-stats --num-threads=$num_threads --num-samples-for-weights=$num_samples_for_weights $dir/$x.ie ark,s,cs:- 'ark,s,cs:gunzip -c $dir/post.JOB.gz|' -|" | sed s/JOB/$j/g`
    done
