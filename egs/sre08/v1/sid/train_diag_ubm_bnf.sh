...
for f in $data/feats.scp $data/vad.scp ${data_bnf}/feats.scp; do
...
all_feats="ark,s,cs:add-deltas $delta_opts scp:$data/feats.scp ark:- | apply-cmvn-sliding --norm-vars=${norm_vars} --center=true --cmn-window=300 ark:- ark:- | select-voiced-frames ark:- scp,s,cs:$data/vad.scp ark:- |"
...
all_feats_bnf="ark,s,cs:apply-cmvn-sliding --norm-vars=${norm_vars} --center=true --cmn-window=300 scp:${data_bnf}/feats.scp ark:- | select-voiced-frames ark:- scp,s,cs:$data/vad.scp ark:- |"
...
if [ $stage -le -2 ]; then
  ...
  $cmd $parallel_opts $dir/log/gmm_init.log \
    append-feats "$all_feats" "$all_feats_bnf" ark:- \| \
...
if [ $stage -le -1 ]; then
  ...
  $cmd $dir/log/gselect.$g.log \
    ...
    "`echo $feats_bnf | sed s/JOB/$g/g`" \
     ark:- \| \
    gmm-gselect --n=$num_gselect $dir/0.dubm ark:- \
...
for x in `seq 0 $[$num_iters-1]`; do
  ...
  for g in $(seq $nj); do
    $cmd $dir/log/acc.$x.$g.log \
      ...
      "`echo $feats_bnf | sed s/JOB/$g/g`" \
       ark:- \| \
      gmm-global-acc-stats "--gselect=ark,s,cs:gunzip -c $dir/gselect.$g.gz|" \
      $dir/$x.dubm ark:- $dir/$x.$g.acc || exit 1 &
  done
  wait;
