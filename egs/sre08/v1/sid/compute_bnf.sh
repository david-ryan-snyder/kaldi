...
case $feat_type in
  ...
  online) feats="ark,s,cs:apply-cmvn-sliding scp:$sdata/JOB/feats.scp ark:- |";;
...
if [ $stage -le 1 ]; then
  ...
  for g in $(seq $nj); do
    $cmd $dir/log/make_bnf_$name.$g.log \
      nnet-compute --use-gpu=${use_gpu} --chunk-size=${chunk_size} $bnf_nnet \
        "`echo $feats | sed s/JOB/$g/g`" ark:- \| \
         copy-feats --compress=true ark:- \
         ark,scp:$archivedir/raw_bnfeat_$name.$g.ark,$archivedir/raw_bnfeat_$name.$g.scp \
         || exit 1 &
  done
  wait
fi
...
if [[ "$N0" != "$N1" ]]; then
  echo "Not all features created for BNF for $name (Original:$N0  BNF:$N1)"
fi

# Concatenate feats.scp into bnf_data
for n in $(seq $nj); do  cat $archivedir/raw_bnfeat_$name.$n.scp; done > $bnf_data/feats.scp

for f in segments spk2utt text utt2spk wav.scp char.stm glm kws reco2file_and_channel stm vad.scp; do
